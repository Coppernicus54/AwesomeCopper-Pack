...
IF NOT EXIST "%cd%\serverstarter-@@serverstarter-libVersion@@.jar" (
	...
	%SYSTEMROOT%\SYSTEM32\bitsadmin.exe /rawreturn /nowrap /transfer starter /dynamic /download /priority foreground https://github.com/BloodyMods/ServerStarter/releases/download/v2.2.0/serverstarter-2.2.0.jar   GOTO MAIN
) ELSE (
   GOTO MAIN
)
